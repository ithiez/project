...
<html>
    <head>
        ...
        <title>Danh sách Chapter Tiểu thuyết</title>
    ...
    <body class="d-flex flex-column h-100">
        <main role="main">
            ...
            $sqlDanhSachChuongTruyen = <<<EOT
      SELECT c.chuong_id, c.chuong_so, c.chuong_ten, c.chuong_noidung, c.truyen_id,
        t.truyen_ten
      FROM chuong c
      JOIN truyen t ON c.truyen_id = t.truyen_id
      WHERE t.truyen_loai = 1
      ORDER BY t.truyen_ten, c.chuong_so;
EOT;

            // 3. Thực thi câu truy vấn SQL để lấy về dữ liệu
            $result = mysqli_query($conn, $sqlDanhSachChuongTruyen);

            // 4. Khi thực thi các truy vấn dạng SELECT, dữ liệu lấy về cần phải phân tích để sử dụng
            // Thông thường, chúng ta sẽ sử dụng vòng lặp while để duyệt danh sách các dòng dữ liệu được SELECT
            // Ta sẽ tạo 1 mảng array để chứa các dữ liệu được trả về
            $dataDanhSachChuongTruyenTranh = [];
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                $dataDanhSachChuongTruyenTranh[] = array(
                    'chuong_id' => $row['chuong_id'],
                    'chuong_so' => $row['chuong_so'],
                    'chuong_ten' => $row['chuong_ten'],
                    'chuong_noidung' => strlen($row['chuong_noidung']) > 50 ? substr($row['chuong_noidung'], 0, 50) . "..." : $row['chuong_noidung'],
                    'truyen_id' => $row['truyen_id'],
                    'truyen_ten' => $row['truyen_ten'],
                );
            }
            // print_r($dataDanhSachChuongTruyenTranh);die;
            ?>
        </div>

        <h1 class="tieu-de">DANH SÁCH CHAPTER TIỂU THUYẾT</h1>
            ...
            <div class="row">
                <div class="col">
                    ...
                    <table class="table table-bordered bg-light text-center">
                        ...
                        <tbody>
                            <?php foreach ($dataDanhSachChuongTruyenTranh as $tap) : ?>
                                <tr>
                                    <td><?= $tap['chuong_id'] ?></td>
                                    <td><?= $tap['chuong_so'] ?></td>
                                    <td><?= $tap['chuong_ten'] ?></td>
                                    <td><?= $tap['chuong_noidung'] ?></td>
                                    ...
                                    <td>
                                        ...
                                        <button type="button" class="btnDelete btn-outline-danger" data-chuong_id="<?= $tap['chuong_id'] ?>">
